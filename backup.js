...
const bz2FilePath = './metadata_tech-test';

async function unzipTarFile() {
	console.log('calledTAr');
	...
	console.log('tarNull', unzipTarFileInstance);
...
async function unzipFile() {
	...
	if (unzipFileInstance === 'successful') {
		const unzipTarFileInstance = await processInstance
			.unzipTarFile(createWriteStream, tar, readTarPath, tarFilePath)
			.catch(err => err);
		if (unzipTarFileInstance === 'successful') {
...
async function fileDownload(url) {
	...
	if (fileDownloadInstance === 'successful') {
		if (urlExtension === 'zip') {
			...
			if (unzipFileInstance === 'successful') {
				const unzipTarFileInstance = await processInstance
					.unzipTarFile(createReadStream, tar, readTarPath, tarFilePath)
					.catch(err => err);
				if (unzipTarFileInstance === 'successful') {
					return unzipFileInstance;
				}
